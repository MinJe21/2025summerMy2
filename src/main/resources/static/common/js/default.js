...
/* list관련 기능 */
function set_search_sdatefdate(obj, obj_target){
	var temp_d = new Date();
...
/* scrollList관련 기능 */
function listenerBeforeScrollList(tbody_id){
	...
	let orderway = $("#search_orderway").val() + "";
	if(isNull(orderway)){ orderway = "desc"; }
	$("#search_orderway").val(orderway);

	let input_cursor_id = $(".input_cursor_id");
	let max_value = null;
	let min_value = null;
	if(input_cursor_id.length > 0){
		max_value = Number($(input_cursor_id[0]).val());
		min_value = Number($(input_cursor_id[0]).val());
		for(let each of input_cursor_id){
			let each_value = Number($(each).val());
			if(each_value > max_value){ max_value = each_value; }
			if(each_value < min_value){ min_value = each_value; }
		}
		if(orderway === "desc"){
			$("#search_cursor").val(min_value);
		} else {
			$("#search_cursor").val(max_value);
		}
	}
}
function listenerAfterScrollList(){
	let font_order = $(".font_order");
	for (let t = 0; t < font_order.length; t++) {
		$(font_order[t]).text((t+1));
	}
	listenerSetSearchKeyword();
}
/* pagedList관련 기능 */
